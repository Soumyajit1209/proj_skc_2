...
const deleteEmployee = async (req, res) => {
    try {
        const connection = await getDbConnection();
        await connection.execute('UPDATE employee_master SET status = "TERMINATED" WHERE emp_id = ? AND company_id = ?', [req.params.id, req.user.company_id]);
...
const getOrders = async (req, res) => {
    try {
        const connection = await getDbConnection();
        const [rows] = await connection.execute(
            'SELECT * FROM sales_order WHERE company_id = ?',
            [req.user.company_id]
        );
        res.json({
            company_id: req.user.company_id,
...
const getAllAttendance = async (req, res) => {
    ...
    try {
        const connection = await getDbConnection();
        let query = 'SELECT * FROM attendance_register WHERE company_id = ?';
